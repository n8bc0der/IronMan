...
public class AppTest {
	...
	public static void testEntityTables() {
		// User user = new User(firstName, lastName, userName, userPassword, userAge,
		// userEmail, gender, dob, userstamp, timestamp, posts, comments);
		...
		// TODO: Create a new user named Ankit with two posts - each post should have at
		// least one comment
	...
	public static void getDetails(long startingIndex, long numberOfRecords) {
		...
		try {
			...
			String sql = "select postDescription from Post p" + " where p.postID>:id and p.postID<:id1";
			...
			// Fetching Username from Starting Index : startingIdex to Number Of Records :
			// numberOfRecords

			String userQuery = "Select userName from User";

			Query query1 = session.createQuery(userQuery).setFirstResult((int) startingIndex)
					.setMaxResults((int) numberOfRecords);
	...
	public static void main(String[] args) {
		...
		// comments and post - see method's body for description

		// #17: populate tables with dummy data
		fillTables();

		// #18 : Implementing Pagination over User & Post table
		getDetails(3L, 4L);
	}
}
